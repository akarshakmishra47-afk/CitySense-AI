document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function checkAuthAndLoad() {
    try {
      ...
      if (!authData.authenticated || authData.user.role !== 'admin_municipal_council') {
        throw new Error('Unauthorized. Only Municipal Council Admins can access this dashboard.');
      }

      const user = authData.user;
      document.getElementById('header-council-name').textContent = user.localBodyName || user.municipalCorp || 'Municipal Council';
    ...
    statusChartInstance = new Chart(statusCtx, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: ['#10B981', '#FBBF24', '#00E5FF', '#FF3366'], // Emerald is primary here
          borderWidth: 0
        }]
      },
