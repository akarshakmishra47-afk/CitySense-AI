document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function checkAuthAndLoad() {
    try {
      ...
      if (!authData.authenticated || authData.user.role !== 'admin_town_council') {
        throw new Error('Unauthorized. Only Town Council Admins can access this dashboard.');
      }

      const user = authData.user;
      document.getElementById('header-town-name').textContent = user.localBodyName || user.municipalCorp || 'Town Council';
    ...
    statusChartInstance = new Chart(statusCtx, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: ['#F59E0B', '#00E5FF', '#10B981', '#FF3366'], // Amber is primary here
          borderWidth: 0
        }]
      },
